...
const Card = ({ data, index, parentField = ""  }) => {
  return (
    <div className="relative" data-tinafield={`${parentField}.${index}`}>
      <div className="absolute top-10 -bottom-0 left-4 right-0 -z-1 bg-accent4"></div>
      <svg className="absolute -right-1.5 -bottom-2" width="50" height="68" viewBox="0 0 50 68" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="flex items-center">
        {data.imageSrc && (
          <img
            ...
            src={data.imageSrc}
            className="w-36 h-36 absolute top-0 rounded-full object-cover"
            data-tinafield={`${parentField}.${index}.image`}
          />
        )}
        <div className="p-6 pt-40 pl-10" >
          {data.label && (
            <p
          ...
          {data.company && (
            <h4
              className="text-xs font-bold text-accent2"
...
export const SpeakerCards = ({ data, parentField = "" }) => {
  return (
    <Section className="max-w-desktop-full mx-auto px-20 py-20 sm:px-10" background={data.background} navigationLabel={data.navigationLabel}>
      ...
      />
      <div className="speaker-cards grid grid-cols-4 gap-8 md:grid-cols-3 sm:grid-cols-2">
        {data.cards && (
